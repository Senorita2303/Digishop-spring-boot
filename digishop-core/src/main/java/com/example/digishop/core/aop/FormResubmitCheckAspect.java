    ...
    private String getParameter(Method method, Object[] args) {
        ...
        for (int i = 0; i < parameters.length; i++) {
            ...
            if (requestBody != null) {
                ...
                String key = com.example.digishop.util.StringUtils.isEmpty(requestParam.value()) ? parameter.getName() : requestParam.value();
                map.put(key, args[i]);
                argList.add(map);
            } else {
                argList.add(args[i]);
