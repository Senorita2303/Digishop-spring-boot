...
public class DigishopServiceImpl implements DigishopService {
	...
	public void removeRememberMe(String username) {
		...
		digishopMapper.removeRememberMe(username);
	}
	...
	public LoginUser checkUserLoginStatus(HttpServletRequest request) {
		try {
			...
			String ssoToken = (String) GlobalCacheHelper
					.get(Constants.CacheKey.ssoTkn.cacheKey().concat(":").concat(ip));
