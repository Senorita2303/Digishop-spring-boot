...
import com.example.digishop.context.exception.ContextErrorCode;
import com.example.digishop.context.exception.ContextException;
 ...
 * It provides methods to retrieve and store context tags based on configuration
 * properties.
 *
 * @author SunTao
 * @since 2024-07-02
...
class CoreContext {
	...
	/**
	 * Stores the value of the given context tag in the context map if it is
	...
	public void put(ContextTag contextTag) {
		...
		if (CACHE_MAP.containsKey(tagName)) {
			...
			boolean isAllowedTag = tags.stream()
					.anyMatch(tag -> tag.toLowerCase(Locale.ROOT).equals(tagName.toLowerCase(Locale.ROOT)));
