package com.example.digishop.sequence.core.snowflake;
...
import com.example.digishop.sequence.core.BaseWorkSequenceGenerator;

import lombok.extern.slf4j.Slf4j;

/**
 * 基于 Twitter 的 snowflake 算法 0 - 0000000000 0000000000 0000000000 0000000000 0 -
 * 000 - 0000000 - 000000000000
 *
 * 1: 1位符号位，由于 long 基本类型在 Java 中是带符号的，最高位是符号位，正数是 0，负数是 1，由于 id 一般是正数，最高位是 0
 * 2-42: 41位时间戳，毫秒数（当前时间戳 - 开始时间戳），41 位的时间戳可以使用 69 年，公式位
 * (1L<<41)/(1000L*60*60*24*365) 43-52: 10位机器位，包括 5 位 datacenterId 和 5 位
 * machineId（也可以是其他组合） 53-64: 12位序列号
 *
 ...
 *
 * 最大峰值型 vs 最小粒度型 通过调短时间戳长度和调长序列号长度，可以解决峰值压力大的情况。
...
public class SnowflakeSequenceGenerator extends BaseWorkSequenceGenerator {
	...
	public SnowflakeSequenceGenerator(long datacenterId, long machineId) {
		...
		log.info("Create snowflake sequence generator: datacenterId={}, machineId={}", this.datacenterId,
				this.machineId);
	}
	...
	public String nextId() {
		...
		return String.valueOf(((currentTime - START_TIME_MILLIS) << TIMESTAMP_LEFT_SHIFT_BITS)
				| (getDatacenterId() << DATACENTER_ID_SHIFT) | (getMachineId() << MACHINE_ID_SHIFT) | seq);
	}
